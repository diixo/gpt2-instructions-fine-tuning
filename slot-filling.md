...
##### Диалоговые системы / голосовые ассистенты (Siri, Alexa, Google Assistant).
...
##### Инструкционные модели (как в твоём случае).
...
* Сначала определяется **намерение** (intent).

"Включи лампочку" → intent = управление устройством.

* Затем извлекаются **слоты**.
...
* 1. *Выполнить действие (изменить состояние)*
* 2. *Проверить результат (лампочка включена?)*
